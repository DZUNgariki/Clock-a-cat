...
public class newcam : MonoBehaviour {
	...
	void Update () 
	{
		if (target)
		{
			...
			Vector3 delta = new Vector3(target.position.x, target.position.y+0.75f,target.position.z) - GetComponent<Camera>().ViewportToWorldPoint(new Vector3(0.5f, point.y, point.z));
